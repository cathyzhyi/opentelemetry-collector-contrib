...
func (bp *batchProcessor) sendItems(measure *stats.Int64Measure) {
	...
	bp.logger.Info("yizhang sendItems before export batch_processor")
...
func (bm *batchMetrics) export(ctx context.Context) error {
	fmt.Println("yizhang batchMetrics export before printStack")
	debug.PrintStack()
	for i := 0; i < bm.metricData.ResourceMetrics().Len(); i++ {
		rm := bm.metricData.ResourceMetrics().At(i)
