...
func dumpRequest(req *monitoringpb.CreateTimeSeriesRequest) {
	fmt.Println("==================yizhang dump TimeSeriesRequst before rpc ================")
	for i, time := range req.TimeSeries {
		fmt.Printf("TimeSeries[%d]: Type %s\n", i, time.Resource.Type)
...
func (c *MetricClient) CreateTimeSeries(ctx context.Context, req *monitoringpb.CreateTimeSeriesRequest, opts ...gax.CallOption) error {
	...
	err := gax.Invoke(ctx, func(ctx context.Context, settings gax.CallSettings) error {
		...
		if err != nil {
			fmt.Printf("yizhang gax.Invoke %s\n", err.Error())
		} else {
			fmt.Printf("yizhang gax.Invoke metricClient no error\n")
		}
		return err
	}, opts...)
	fmt.Println("yizhang CreatetimeSeries testing")
	return err
}
