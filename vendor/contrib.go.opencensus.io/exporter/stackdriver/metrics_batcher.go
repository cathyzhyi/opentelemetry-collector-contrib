...
func dumpRequst(req *monitoringpb.CreateTimeSeriesRequest) {
	fmt.Println("==================yizhang sendReq before createTimeSeries metrics_batcher ================")
	for i, time := range req.TimeSeries {
		fmt.Printf("TimeSeries[%d]: %s\n", i, time.Resource.Type)
		for k, v := range time.Resource.Labels {
			fmt.Printf("%s : %s\n", k, v)
		}
	}
	fmt.Println("==================yizhang finished ================")
}
...
func sendReq(ctx context.Context, c *monitoring.MetricClient, req *monitoringpb.CreateTimeSeriesRequest) (int, error) {
	...
	fmt.Printf("yizhang sendReq after CreateTimeSeries has err: %s\n", err.Error())
