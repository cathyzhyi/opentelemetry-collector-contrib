...
func (se *statsExporter) PushMetricsProto(ctx context.Context, node *commonpb.Node, rsc *resourcepb.Resource, metrics []*metricspb.Metric) (int, error) {
	...
	for _, metric := range metrics {
		...
		fmt.Println("yizhang PushMetricsProto metric_proto.go")
...
func DumpResource(resource *resourcepb.Resource) {
	switch resource.Type {
	case knativeBrokerType:
		fmt.Println("broker:")
	case knativeTriggerType:
		fmt.Println("trigger:")
	default:
...
func dumpMappedResource(resource *monitoredrespb.MonitoredResource) {
	switch resource.Type {
	case knativeBrokerType:
		fmt.Println("broker:")
	case knativeTriggerType:
...
func (se *statsExporter) getResource(rsc *resourcepb.Resource, metric *metricspb.Metric, seenRscs map[*resourcepb.Resource]*monitoredrespb.MonitoredResource) *monitoredrespb.MonitoredResource {
	var resource = rsc
	fmt.Println("yizhang getResource stackdriver exporter dumpStack")
	debug.PrintStack()
	fmt.Println("yizhang getResource dump rsc")
	DumpResource(rsc)
	if metric.Resource != nil {
		resource = metric.Resource
	}
	fmt.Println("yizhang getResource dump resource")
	DumpResource(rsc)
	...
	fmt.Println("yizhang getResource dump mapped resource")
	dumpMappedResource(mappedRsc)
	return mappedRsc
}
